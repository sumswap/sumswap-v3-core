...
import { MockTimeSummaSwapV3Manager, Quoter, TestERC20 } from '../typechain'
...
describe('Quoter', () => {
  ...
  describe('quotes', () => {
    beforeEach(async () => {
      await createTestPool(nft, wallet, tokens[0].address, tokens[1].address);
    ...
    describe('#quoteExactInput', () => {
      it('0 -> 1', async () => {
        const quote = await quoter.callStatic.quoteExactInput(
          wallet.address,
          encodePath([tokens[0].address, tokens[1].address], [FeeAmount.LOW]),
          1000
        )
